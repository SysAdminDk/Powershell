...
if ($GPO.Count -gt 1) {
    throw "Multiple Domain controller MSFT GPO's found, please change query to onlky return one"
    break
}
...
$Backup = $GPO | Backup-GPO -Path "$($env:TEMP)\MSFT"
...
# --
Import-GPO -BackupId $GPOBackupFolder.Name -Path "$($env:TEMP)\MSFT" -TargetName $GPO.DisplayName -CreateIfNeeded | Out-Null


# --
# Cleanup GPO backup
# --
Remove-Item -Path "$($env:TEMP)\MSFT" -Confirm:$false -Recurse -Force
